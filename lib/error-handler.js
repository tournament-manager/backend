...
module.exports = function (err, res) {
  ...
  case msg.includes('cast'): return res.status(404).send(`${err.name}: ${err.message}`);
  case msg.includes('bad'): return res.status(400).send(`${err.name}: ${err.message}`);
  case msg.includes('multi'): return res.status(401).send(`${err.name}: ${err.message}`);
